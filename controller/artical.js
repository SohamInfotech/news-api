...
var todoModel = require('../model/artical')

exports.createartical = async (req, res) => {
    try {

        const news = await todoModel.create(req.body)
        res.status(200).json({
            status: 'Success',
...
exports.showartical = async (req, res) => {
    try {
        const news = await todoModel.find().populate('userID');
        res.status(200).json({
            status: "Success",
